...
class ContactData extends Component {
    ...
    executeScroll = () => this.formRef.current.scrollIntoView()
        ...
        if(this.state.ordered){
            modal = 
                <Modal show={this.state.ordered} >
                    <h2>Thank you for your order! It will be delivered shortly. You will now be taken back to the home page.</h2>
                </Modal>
        }

        return (
            <div className={classes.ContactData}>
                <h4 ref={this.formRef}>Enter your Contact Data</h4>
                {form}
                {modal}
            </div>
        );
...
const mapDispatchToProps = dispatch => {
    return {
        purchaseStart: (orderData, token) => dispatch(orderActions.purchaseStart(orderData, token)),
